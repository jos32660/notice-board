...
# 예제
INSERT INTO gbook SET comment='안녕하세요', wtime='2019-09-03 12:26:22';
```
...
# 예제
INSERT INTO gbook ('comment', 'wtime') VALUES ('안녕하세요', '2019-09-03 12:26:22');
```
...
SELECT * FROM gbook WHERE comment = 'Hi' --단어만 찾아서 검색할때
SELECT * FROM gbook WHERE comment LIKE = '%Hi%' ORDER BY id DESC --%안에 포함된 단어를 찾아서 검색할때 
...
-- 레코드의 갯수를 가져오기
SELECT conut(id) FROM gbook
```


### 데이터 삭제
```sql
--삭제 SQL - WHERE절이 없으면 모든 데이터가 삭제되므로 박스를 구해야 함.
